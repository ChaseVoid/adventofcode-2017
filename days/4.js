...
.map(line => line.reduce((all, item) => {all[item] = (all[item] || 0) + 1
return all}, {}))
.reduce(
  (accumulator, currentValue, currentIndex, array) => {
    //console.log(currentValue, Object.values(currentValue).reduce((sum, i) => sum + i, 0), Object.values(currentValue).length )
    return accumulator + (Object.values(currentValue).reduce((sum, i) => sum + i, 0) === Object.values(currentValue).length ? 1 : 0)
  }, 0
)
)
...
const phrases = `pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
vxjnf fzqitnj uyfck blnl impo kxoow nngd worcm bdesehw
caibh nfuk kfnu llfdbz uxjty yxjut jcea
qiho qif eupwww avyglnj nxzotsu hio lws
xjty usocjsh pivk qnknunc yjcgh bwya djw zpyr
